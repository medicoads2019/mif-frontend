...
export default function CreateClient() {
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (data?.success) {
        message.success(data.message || "Client created successfully");
        form.resetFields();
        setProfileFile(null);
        setPreview(null);
        navigate("/clients");
      } else {
        message.error(data?.message || "Client creation failed");
      }
    } catch (error) {
      message.error(
    ...
    <>
      ...
      <Card>
        ...
        <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          {/* ── BUSINESS INFO ── */}
          <Divider orientation="left">Business Information (Optional)</Divider>
          <Row gutter={16}>
            <Col xs={24} sm={12}>
