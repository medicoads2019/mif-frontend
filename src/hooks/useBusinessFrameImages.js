...
export default function useBusinessFrameImages() {
  const [activeImages, setActiveImages] = useState([]);
  const [deletedImages, setDeletedImages] = useState([]);
  const [selectedImage, setSelectedImage] = useState(null);

  /* ===============================
     LOAD IMAGES BY BUSINESS FRAME
  =============================== */

  const loadAllImages = async (businessFrameId) => {
    try {
      if (!businessFrameId) {
        setActiveImages([]);
        setDeletedImages([]);
      ...
      const res = await getImagesByBusinessFrame(businessFrameId);
