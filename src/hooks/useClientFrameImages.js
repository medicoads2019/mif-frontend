...
export default function useClientFrameImages() {
  const [activeImages, setActiveImages] = useState([]);
  const [deletedImages, setDeletedImages] = useState([]);
  const [selectedImage, setSelectedImage] = useState(null);

  /* ===============================
     LOAD IMAGES BY CLIENT FRAME
  =============================== */

  const loadAllImages = async (clientFrameId) => {
    try {
      if (!clientFrameId) {
        setActiveImages([]);
        setDeletedImages([]);
      ...
      const res = await getImagesByClientFrame(clientFrameId);
