...
import {
  ...
  restoreImage,
  hardDeleteImage,
} from "../api/businessimageApi";

export default function useBusinessImages() {
  const [activeImages, setActiveImages] = useState([]);
  const [deletedImages, setDeletedImages] = useState([]);
  const [selectedImage, setSelectedImage] = useState(null);

  /* ===============================
     LOAD IMAGES BY BUSINESS
  =============================== */

  const loadAllImages = async (businessId) => {
    try {
      if (!businessId) {
        setActiveImages([]);
        setDeletedImages([]);
      ...
      const res = await getImagesByBusiness(businessId);
